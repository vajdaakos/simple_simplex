...
void exercise_load_plugin_loader::load_plugins(QDir root_dir)
{
    root_dir.cd("plugins");
    root_dir.cd("exercise_load_plugins");
    foreach (QString fileName, root_dir.entryList(QDir::Files))
    {
        ...
        if (plugin)
        {
            ...
            if (exercise_load_plugin_interface)
            {
                ...
                QVariant v = qVariantFromValue(reinterpret_cast<void *>(exercise_load_plugin_interface));
